...
class WishlistTest {
    ...
    void testWishlistConstructorAndGetters() {
        ...
        Wishlist wishlist = new Wishlist(1, "Christmas wishlist", "Mikkel", "For Christmas", List.of(wish));

        assertEquals(1, wishlist.getWishlistId());
        assertEquals("Christmas wishlist", wishlist.getWishlistName());
        assertEquals("Mikkel", wishlist.getOwnerName());
        assertEquals("For Christmas", wishlist.getDescription());
        assertEquals(wish, wishlist.getWishes().get(0));
    }
}
